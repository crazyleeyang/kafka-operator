...
func GetMaxHeap(cluster spec.Kafkacluster) *resource.Quantity {
...
func GetMaxHeapJavaString(cluster spec.Kafkacluster) string {
	return "-Xmx" + GetMaxHeap(cluster).String()
}

func GenerateKafkaOptions(cluster spec.Kafkacluster) []v1.EnvVar {
	...
	staticOptions := []v1.EnvVar{
		...
		v1.EnvVar{
			...
			Value: GetMaxHeapJavaString(cluster),
		},
		v1.EnvVar{
